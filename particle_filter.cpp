...
void ParticleFilter::prediction(double delta_t, double std_pos[], double velocity, double yaw_rate) {
    ...
    for(int part_i = 0; part_i < num_particles; part_i++){
        //create normal distribution to grab from
        normal_distribution<double> dist_x(particles[part_i].x, std_x);
        normal_distribution<double> dist_y(particles[part_i].y, std_y);
        normal_distribution<double> dist_yaw(particles[part_i].theta, std_yaw);

        //predict particles new prediction using velocity, yaw_rate, x and y with noise and yaw with noise
        double noisy_x, noisy_y, noisy_yaw;

        noisy_x = dist_x(gen);
        noisy_y = dist_y(gen);
        noisy_yaw = dist_yaw(gen);


        //predict particle states
        particles[part_i].x = noisy_x + (velocity / yaw_rate) * (sin(noisy_yaw + (yaw_rate * delta_t)) - sin(noisy_yaw));
        particles[part_i].y = noisy_y + (velocity / yaw_rate) * (cos(noisy_yaw) - (cos(noisy_yaw + (yaw_rate * delta_t))));
        particles[part_i].theta = noisy_yaw + (yaw_rate * delta_t);
